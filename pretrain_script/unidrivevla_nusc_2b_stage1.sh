#!/bin/bash
# VLM Pretraining for UniDriveVLA-Base (2B) on nuScenes driving QA data
# Run from the qwenvl3/ directory: bash ../pretrain_script/unidrivevla_nusc_2b_stage1.sh
...
SAVE_DIR=${SAVE_DIR:-"megatron_output/UniDriveVLA_Nusc_Base_Stage1"}
# ==============================
...
torchrun "${DISTRIBUTED_ARGS[@]}" \
    ...
    --val_dataset "${DATASET_ROOT}/nuscenes_traj_val.jsonl" \
    ...
    --no_save_rng true \
    --dataset_num_proc 32 \
    --attention_backend flash > unidrivevla-pretrain-2b-nusc.txt 2>&1
