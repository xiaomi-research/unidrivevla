...
class BEVRender:
    ...
    def draw_motion_pred(self, result, top_k=3):
        ...
        for i in range(result['labels_3d'].shape[0]):
            ...
            traj_score = result['trajs_score'][i].float().numpy()
            traj = result['trajs_3d'][i].float().numpy()
            num_modes = len(traj_score)
            center = bboxes[i, :2][None, None].repeat(num_modes, 1, 1).float().numpy()
