...
if __name__ == '__main__':
    ...
    bevformer_results = mmcv.load('test/bevformer_base/Fri_Oct_10_21_55_05_2025/pts_bbox/results_nusc.json')
    sample_token_list = list(bevformer_results['results'].keys())
    os.makedirs('output/bev', exist_ok=True)
    for id in range(0, 10):
        out_path=os.path.join('output/bev', sample_token_list[id])
        render_sample_data(sample_token_list[id], pred_data=bevformer_results, out_path=out_path)
