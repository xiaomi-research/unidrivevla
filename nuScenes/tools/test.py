...
from mmcv.runner import (get_dist_info, init_dist, load_checkpoint, load_deepspeed_checkpoint,
                         wrap_fp16_model)
...
def main():
    ...
    if rank == 0:
        if args.out:
            ...
            # Save pred_trajs_dict.pkl for nuScenes offline evaluation (evaluation_new.py)
            # Format: {sample_token: np.array(T, 2)} where T=6 (3s at 2Hz)
            ...
            print(f'pred_trajs_dict saved to {pred_trajs_path} ({len(pred_trajs_dict)} samples)')
