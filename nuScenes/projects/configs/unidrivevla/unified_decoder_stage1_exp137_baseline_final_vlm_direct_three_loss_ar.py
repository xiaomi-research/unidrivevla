...
ar_dataset_cfg = dict(
    enabled=True,
    jsonl_path='/high_perf_store3/world-model/yongkangli/UniDriveVLA/nuscenes_planning_qa_train_only_command_his_traj.jsonl',
    data_root='./',
    samples_per_gpu=1,  # AR batch size per GPU (降低以节省显存)
    workers_per_gpu=2,
    max_length=8192,    # Maximum sequence length for AR
    fix_image_size=(960, 544),  # Optional: (width, height) for fixed image size
)