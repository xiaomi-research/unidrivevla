...
class UnifiedPerceptionDecoder(BaseModule):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if 'motion' in task_select:
            ...
            self.motion_instance_bank = build(motion_instance_bank, PLUGIN_LAYERS)
            self.motion_anchor_encoder = build(motion_anchor_encoder, POSITIONAL_ENCODING)
            self.motion_deformable = nn.ModuleList([build(motion_deformable, ATTENTION) for _ in range(num_deform)])
    ...
    def forward_stage1(self, feature_maps, metas):
        ...
        if not self.query_select:
            ...
            _t = metas['projection_mat'].dtype
            empty = lambda *s: torch.zeros(*s, device=_d, dtype=_t)
            self._stage1_cache = dict(
                det_feat=empty(B, 0, self.embed_dims),
                map_feat=empty(B, 0, self.embed_dims),
                ego_feat=empty(B, 0, self.embed_dims),
                det_anchor=empty(B, 0, 11),
                map_anchor=empty(B, 0, 20 * 3),
                ego_anchor=empty(B, 0, 11),
                det_anchor_embed=empty(B, 0, self.embed_dims),
                map_anchor_embed=empty(B, 0, self.embed_dims),
                ego_anchor_embed=empty(B, 0, self.embed_dims),
                time_interval=None,
                map_points_embed=None,
                motion_mode_query=None,
                temp_det_feat=None, temp_map_feat=None, temp_ego_feat=None,
                temp_det_anchor_embed=None, temp_map_anchor_embed=None,
                temp_ego_anchor_embed=None, temp_map_points_embed=None,
                with_temp=False,
                num_temp_anchor_cumsum=[0, 0, 0, 0],
                metas=metas,
                feature_maps=feature_maps,
                det_cls_last=None, map_cls_last=None,
            )
            return dict(
                det_instance_feature=empty(B, 0, self.embed_dims),
                det_anchor=empty(B, 0, 11),
                det_anchor_embed=empty(B, 0, self.embed_dims),
                det_classifications=[], det_predictions=[], det_qualities=[],
                map_instance_feature=empty(B, 0, self.embed_dims),
                map_anchor=empty(B, 0, 20 * 3),
                map_anchor_embed=empty(B, 0, self.embed_dims),
                map_classifications=[], map_predictions=[], map_qualities=[],
                ego_instance_feature=empty(B, 0, self.embed_dims),
                ego_anchor=empty(B, 0, 11),
                ego_anchor_embed=empty(B, 0, self.embed_dims),
                ego_status_list=[],
        ...
        if 'det' in self.query_select:
            det_feat, det_anchor, temp_det_feat, temp_det_anchor, time_interval = \
                self.det_instance_bank.get(B, metas, dn_metas=self.det_sampler.dn_metas if hasattr(self.det_sampler, 'dn_metas') else None)
        if 'map' in self.query_select:
            map_feat, map_anchor, temp_map_feat, temp_map_anchor, _ = \
                self.map_instance_bank.get(B, metas, dn_metas=self.map_sampler.dn_metas if hasattr(self.map_sampler, 'dn_metas') else None)
        if 'ego' in self.query_select:
            ego_feat, ego_anchor, temp_ego_feat, temp_ego_anchor = \
                self.ego_instance_bank.get(B, metas, feature_maps)
            ...
            if op == "concat":
                ...
                if "det" in self.task_select:
                    ...
                    if len(det_pred_list) == self.num_single_frame_decoder:
                        det_feat, det_anchor = self.det_instance_bank.update(
                ...
                if "map" in self.task_select:
                    ...
                    if len(map_pred_list) == self.num_single_frame_decoder:
                        map_feat, map_anchor = self.map_instance_bank.update(
    ...
    def forward_stage2(self, vlm_enhanced, feature_maps, metas):
        ...
        if not self.query_select:
            ...
            empty = lambda *s: torch.zeros(*s, device=_d, dtype=_t)
            return dict(
                det_instance_feature=empty(metas['projection_mat'].shape[0], 0, self.embed_dims),
                det_anchor=None, det_classifications=[], det_predictions=[], det_qualities=[],
                map_instance_feature=empty(metas['projection_mat'].shape[0], 0, self.embed_dims),
                map_anchor=None, map_classifications=[], map_predictions=[], map_qualities=[],
                ego_instance_feature=empty(metas['projection_mat'].shape[0], 0, self.embed_dims),
                ego_status_list=[],
            ...
            if ego_gt is not None:
                ...
                for status_pred in all_ego_status:
                    ...
                    ego_loss = ego_loss + self.loss_ego_status(
                        status_pred.squeeze(1), ego_gt_t
                    )
                losses['loss_ego_status'] = ego_loss
            else:
                losses['loss_ego_status'] = zero

        if "motion" in self.task_select and self.motion_sampler is not None:
    ...
    def post_process(self, stage2_outs):
        ...
        if hasattr(self, 'det_decoder') and self.det_decoder is not None:
            ...
            if len(det_cls_list) > 0:
                ...
                if self.with_instance_id and hasattr(self, 'det_instance_bank'):
                    try:
                        instance_id = self.det_instance_bank.get_instance_id(
                            det_cls_list[-1],
                            det_pred_list[-1],
                            self.det_decoder.score_threshold
                        )
        ...
        if hasattr(self, 'map_decoder') and self.map_decoder is not None:
        ...
        if (hasattr(self, 'motion_decoder') and self.motion_decoder is not None
                and det_result is not None and len(det_cls_list) > 0):
            motion_cls_list = stage2_outs.get('motion_cls_list', [])
            motion_reg_list = stage2_outs.get('motion_reg_list', [])
            if len(motion_cls_list) > 0 and len(motion_reg_list) > 0:
                motion_output = {
                    "classification": motion_cls_list,
                    "prediction": motion_reg_list,
                }
                ...
                for i, m in enumerate(motion_result):
                    det_result[i]["trajs_3d"] = m["trajs_3d"]
                    det_result[i]["trajs_score"] = m["trajs_score"]
