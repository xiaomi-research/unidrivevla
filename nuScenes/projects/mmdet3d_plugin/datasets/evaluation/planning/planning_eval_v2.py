...
class PlanningMetric():
    ...
    def update(self, trajs, gt_trajs, gt_trajs_mask, fut_boxes, safe_incomplete=False):
        ...
        obj_coll_sum, obj_box_coll_sum = self.evaluate_coll(trajs[:,:,:2], gt_trajs[:,:,:2], fut_boxes, safe_incomplete)
...
def _print_planning_results(planning_results, logger, title):
...
def planning_eval(results, eval_config, logger):
    ...
    # Create two metric instances
    planning_metrics_strict = PlanningMetric() # Skips incomplete GT
    planning_metrics_loose = PlanningMetric()  # Does NOT skip incomplete GT (SOTA-aligned)
    ...
    for i, data in enumerate(tqdm(dataloader)):
        ...
        # Update loose metric unconditionally; safe_incomplete=True handles missing fut_boxes timesteps
        planning_metrics_loose.update(pred_sdc_traj[:, :6, :2], sdc_planning[0,:, :6, :2], sdc_planning_mask[0,:, :6, :2], fut_boxes, safe_incomplete=True)

        # Update strict metric only for complete GTs; safe_incomplete=False matches old behaviour exactly
        if sdc_planning_mask.all():
            planning_metrics_strict.update(pred_sdc_traj[:, :6, :2], sdc_planning[0,:, :6, :2], sdc_planning_mask[0,:, :6, :2], fut_boxes, safe_incomplete=False)

    # Compute and print results for both

    # Strict (skip incomplete)
    planning_results_strict = planning_metrics_strict.compute()
    _print_planning_results(planning_results_strict, logger, "Planning Metrics (Strict: Skip Incomplete GT)")

    # Loose (SOTA-aligned)
    planning_results_loose = planning_metrics_loose.compute()
    metric_dict_loose = _print_planning_results(planning_results_loose, logger, "Planning Metrics (Loose: SOTA-aligned, No Skip)")

    planning_metrics_strict.reset()
    planning_metrics_loose.reset()

    # Return the SOTA-aligned metric dict as the main result
    return metric_dict_loose
