...
class NuScenes3DDataset(Dataset):
    ...
    def __init__(
        ...
        if vad_ann_file and os.path.exists(vad_ann_file):
            ...
            data = mmcv.load(vad_ann_file, file_format='pkl')
            self.vad_infos_map = {info['token']: info for info in data['infos']}
        else:
            self.vad_infos_map = None
    ...
    def get_data_info(self, index):
        ...
        scene_name = self.nusc.get('scene', info['scene_token'])['name']
        input_dict = dict(
            token=info["token"],
    ...
    def get_ann_info(self, index):
        ...
        # Add historical trajectory from VAD infos if available
        if self.vad_infos_map is not None:
            token = info['token']
            if token in self.vad_infos_map:
                vad_info = self.vad_infos_map[token]
                if 'gt_ego_his_trajs' in vad_info:
                    anns_results['gt_ego_his_trajs'] = vad_info['gt_ego_his_trajs']
        ...
        for sample_id, det in enumerate(mmcv.track_iter_progress(results)):
            ...
            for i, box in enumerate(boxes):
                ...
                if 'trajs_3d' in det['img_bbox']:
                    trajs_val = det['img_bbox']['trajs_3d'][i].float().numpy()
                else:
                    # Model does not predict agent motion; use dummy zeros
                    trajs_val = np.zeros((6, 12, 2), dtype=np.float32)
                nusc_anno.update(dict(trajs=trajs_val))
                annos.append(nusc_anno)
            nusc_annos[sample_token] = annos
        nusc_submissions = {
            "meta": self.modality,
    ...
    def evaluate(
        ...
        # Some entrypoints (e.g., tools/test.py) may not pass work_dir; fall back to a temp directory.
        if self.work_dir is None:
            if jsonfile_prefix is not None:
                self.work_dir = jsonfile_prefix
            else:
                tmp_dir = tempfile.TemporaryDirectory()
                self.work_dir = tmp_dir.name
        ...
        if eval_mode['with_det']:
            ...
            for metric in ["detection", "tracking"]:
                ...
                if tracking and not self.tracking:
                    continue
                result_files, tmp_dir_fmt = self.format_results(
        ...
        if "L2" in results_dict:
            obj_box_col = results_dict["obj_box_col"]
            if isinstance(obj_box_col, list):
                obj_box_col = obj_box_col[0] if len(obj_box_col) > 0 else 0
            metric_str += f'obj_box_col: {(obj_box_col*100):.3f}%\n'
...
def output_to_nusc_box(detection, threshold=None):
    ...
    if threshold is not None:
        if "cls_scores" in detection:
            cls_scores = detection["cls_scores"]
            if cls_scores.dtype == torch.bfloat16:
                cls_scores = cls_scores.float()
            mask = cls_scores.numpy() >= threshold
        else:
    ...
    if hasattr(box3d, "gravity_center"):
        if box3d.tensor.dtype == torch.bfloat16:
            box3d.tensor = box3d.tensor.float()
        box_gravity_center = box3d.gravity_center.numpy()
        box_dims = box3d.dims.numpy()
        nus_box_dims = box_dims[:, [1, 0, 2]]
        box_yaw = box3d.yaw.numpy()
    else:
        if box3d.dtype == torch.bfloat16:
            box3d = box3d.float()
        box3d = box3d.numpy()
