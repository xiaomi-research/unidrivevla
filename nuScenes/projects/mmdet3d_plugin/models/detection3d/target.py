...
class SparseBox3DTarget(BaseTargetWithDenoising):
    ...
    def sample(
        ...
        for i, (pred_idx, target_idx) in enumerate(indices):
            ...
            # Match dtypes for index_put: output_* tensors follow box_pred dtype.
            # box_target/weights may come from fp32 GT tensors while box_pred can be bf16.
            output_box_target[i, pred_idx] = box_target[i][target_idx].to(dtype=output_box_target.dtype)
            output_reg_weights[i, pred_idx] = instance_reg_weights[i][target_idx].to(dtype=output_reg_weights.dtype)
