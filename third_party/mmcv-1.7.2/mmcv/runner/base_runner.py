...
import deepspeed
from deepspeed.runtime.zero.stage_1_and_2 import DeepSpeedZeroOptimizer as ZeROOptimizer
from deepspeed.runtime.zero.stage3 import DeepSpeedZeroOptimizer_Stage3 as ZeROOptimizerStage3
import mmcv
from ..parallel import is_module_wrapper
from .checkpoint import load_checkpoint, load_deepspeed_checkpoint
...
class BaseRunner(metaclass=ABCMeta):
    ...
    def __init__(self,
        ...
        and not isinstance(optimizer, ZeROOptimizer) \
        and not isinstance(optimizer, ZeROOptimizerStage3) \
        and not isinstance(optimizer, deepspeed.runtime.fp16.unfused_optimizer.FP16_UnfusedOptimizer):
            raise TypeError(
    ...
    def current_lr(self) -> Union[List[float], Dict[str, List[float]]]:
        ...
        or isinstance(self.optimizer, ZeROOptimizer) \
        or isinstance(self.optimizer, ZeROOptimizerStage3) \
        or isinstance(self.optimizer, deepspeed.runtime.fp16.unfused_optimizer.FP16_UnfusedOptimizer):
    ...
    def load_deepspeed_checkpoint(
        ...
        custom_load_fn=None):
        return load_deepspeed_checkpoint(
            self.model,
            load_dir, 
            tag=tag, 
    ...
    def register_checkpoint_hook(
        ...
        if isinstance(checkpoint_config, dict):
            if checkpoint_config.get('deepspeed'):
                assert isinstance(self.model, mmcv.parallel.MMDeepSpeedEngine), \
                'deepspeed flag in checkpoint_config should only be used for MMDeepspeedEngine'
                checkpoint_config.setdefault('type', 'DeepspeedCheckpointHook')
                hook = mmcv.build_from_cfg(checkpoint_config, HOOKS)
                del checkpoint_config['deepspeed']
            else:
                assert not isinstance(self.model, mmcv.parallel.MMDeepSpeedEngine), \
                'Please set deepspeed=True in checkpoint_config if using the MMDeepspeedEngine'
