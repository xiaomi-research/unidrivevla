...
def _get_ckpt_name(checkpoints_path, tag, mp_placeholder=None):
...
def _get_all_ckpt_names(checkpoints_path, tag):
    # It is required that (checkpoints_path, tag) are consistent among all ranks.
    ckpt_file_pattern = _get_ckpt_name(checkpoints_path, tag, mp_placeholder="*")
...
def _process_to_deepspeed_ckpt_format(load_dir, tag):
    ckpt_list = _get_all_ckpt_names(load_dir, tag)
...
def load_deepspeed_checkpoint(
    ...
    custom_load_fn=None):
    assert isinstance(model, mmcv.parallel.MMDeepSpeedEngine), "this method should only be used for MMDeepSpeedEngine"
    _ , checkpoint = model.load_checkpoint(load_dir, 
                          tag=tag, 
...
def save_deepspeed_checkpoint(
    ...
    assert isinstance(model, mmcv.parallel.MMDeepSpeedEngine), "this method should only be used for MMDeepSpeedEngine"
    model.save_checkpoint(filepath, tag=tag, client_state=client_state, save_latest=save_latest)