...
from .deformable_aggregation_a800 import DeformableAggregationFunctionA800
...
def deformable_aggregation_function(
    ...
    if device_name == 'NVIDIA A800 80GB PCIe':
        return DeformableAggregationFunctionA800.apply(
            feature_maps,
            spatial_shape,
            scale_start_index,
            sampling_location,
...
def feature_maps_format(feature_maps, inverse=False):
    if inverse:
        ...
        split_size = split_size.to(torch.int64).cpu().numpy().tolist()
        ...
        spatial_shape = spatial_shape.to(torch.int64).cpu().numpy().tolist()
        mc_ms_feat = []
        shape_index = 0
