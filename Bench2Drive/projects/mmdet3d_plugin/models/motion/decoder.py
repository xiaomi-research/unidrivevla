...
def box3d_to_corners_gpu(box3d):
    box3d = box3d.detach()
    corners_norm = np.stack(np.unravel_index(np.arange(8), [2] * 3), axis=1)
    corners_norm = corners_norm[[0, 1, 3, 2, 4, 5, 7, 6]]
    # use relative origin [0.5, 0.5, 0]
    corners_norm = corners_norm - np.array([0.5, 0.5, 0.5])
    corners_norm = torch.tensor(corners_norm, device=box3d.device, dtype=torch.float32)
    corners = box3d[:, None, [W, L, H]] * corners_norm.reshape([1, 8, 3])

    # rotate around z axis
    rot_cos = torch.cos(box3d[:, YAW])
    rot_sin = torch.sin(box3d[:, YAW])
    rot_mat = torch.tile(torch.eye(3, device=box3d.device)[None], (box3d.shape[0], 1, 1))
    rot_mat[:, 0, 0] = rot_cos
    rot_mat[:, 0, 1] = -rot_sin
    rot_mat[:, 1, 0] = rot_sin
    rot_mat[:, 1, 1] = rot_cos
...
class HierarchicalPlanningDecoder(object):
    ...
    def decode(
        ...
        classification = classification.reshape(bs, self.ego_fut_cmd, self.ego_fut_mode)
        prediction = prediction.reshape(bs, self.ego_fut_cmd , self.ego_fut_mode, self.ego_fut_ts, 2).cumsum(dim=-2)
...
@BBOX_CODERS.register_module()
class SparseMotionDecoder(SparseBox3DDecoder):
    def __init__(self):
