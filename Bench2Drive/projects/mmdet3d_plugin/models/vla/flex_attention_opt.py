...
def _unidrive_mask_mod(prefix_len: int, perception_len: int, suffix_len: int):
    p = int(prefix_len)
    ...
    def mask_mod(b, h, q_idx, kv_idx):
        ...
        q_is_prefix = q_idx < p
        q_is_perception = (q_idx >= p) & (q_idx < pp)
        q_is_suffix = q_idx >= pp

        mask_prefix = q_is_prefix & (kv_idx <= q_idx)
        mask_perception = q_is_perception & (kv_idx < pp)
        mask_suffix = q_is_suffix

        return in_range & (mask_prefix | mask_perception | mask_suffix)
...
def build_blockmask_unidrive(
    *,
    ...
    mask_mod = _unidrive_mask_mod(prefix_len, perception_len, suffix_len)
