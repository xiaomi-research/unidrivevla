...
class Qwen3VLWithExpertModel(nn.Module):
    def __init__(
        ...
        self.processor = AutoProcessor.from_pretrained(pretrained_path, use_fast=False)
    ...
    def forward(
        ...
        q_len_rounded: Optional[int] = None,
        deepstack_visual_embeds=None,
        visual_pos_masks=None,
    ):
        ...
        if len(inputs_embeds) == 3 and inputs_embeds[1] is None and inputs_embeds[2] is None:
            prefix_output = self.qwen3_vl.language_model.forward(
                inputs_embeds=inputs_embeds[0].to(self.qwen3_vl.language_model.dtype),
            ...
            middle_output = self.qwen3_perception_expert.forward(
                inputs_embeds=inputs_embeds[1].to(self.qwen3_perception_expert.dtype),
                attention_mask=attention_mask,
                position_ids=position_ids,
                past_key_values=past_key_values,
                use_cache=use_cache,
            ...
            suffix_output = self.qwen3_action_expert.forward(
                inputs_embeds=inputs_embeds[2].to(self.qwen3_action_expert.dtype),
                attention_mask=attention_mask,
                position_ids=position_ids,
                past_key_values=past_key_values,
                use_cache=use_cache,
            ...
            if use_gradient_checkpointing:
                outputs_embeds = torch.utils.checkpoint.checkpoint(
                    compute_final_norms, inputs_embeds, use_reentrant=False, preserve_rng_state=False
                )
            else:
