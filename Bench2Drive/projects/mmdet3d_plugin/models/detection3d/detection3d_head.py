...
class Sparse4DHead(BaseModule):
    def __init__(
        ...
        **kwargs,
        ...
        # Optional separate refinement module for VLM-fused features.
        self.vlm_refine_layer = build(vlm_refine_layer, PLUGIN_LAYERS)
        ...
        self._refine_layer_idx = -1
        for idx, op in enumerate(self.operation_order):
            if op == "refine":
                self._refine_layer_idx = idx
    ...
    def forward_refine(self, vlm_fused_feature, anchor, time_interval=None):
        if self._refine_layer_idx == -1:
            raise RuntimeError("No refine layer found in Sparse4DHead")
        anchor_embed = self.anchor_encoder(anchor)

        refine_layer = self.vlm_refine_layer
        if refine_layer is None:
            refine_layer = self.layers[self._refine_layer_idx]

        final_anchor, final_cls, final_qt = refine_layer(
            instance_feature=vlm_fused_feature,
            anchor=anchor,
            anchor_embed=anchor_embed,
            time_interval=time_interval,
            return_cls=True,
        )
        return final_anchor, final_cls, final_qt
    ...
    def loss(self, model_outs, data, feature_maps=None):
        ...
        for decoder_idx, (cls, reg) in enumerate(
            ...
            # mmcv sigmoid_focal_loss cuda kernel does not support BF16; run loss in FP32.
            cls_loss = self.loss_cls(
                cls.flatten(end_dim=1)[dn_valid_mask].float(),
                dn_cls_target,
