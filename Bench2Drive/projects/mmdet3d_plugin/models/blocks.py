...
class DeformableFeatureAggregation(BaseModule):
    def __init__(
        ...
        use_points_embed=False,
        use_anchor_embed=False,
        residual_mode="add",
    ):
        ...
        self.use_points_embed = use_points_embed
        ...
        if use_points_embed:
            kps_generator["with_points_embed"] = use_points_embed
        if use_anchor_embed:
            kps_generator["with_anchor_embed"] = use_anchor_embed
        self.kps_generator = build_from_cfg(kps_generator, PLUGIN_LAYERS)
        self.kps_pts = self.kps_generator.num_pts
        self.num_pts = self.kps_pts * self.num_sample if use_points_embed else self.kps_pts
        ...
        input_dims = embed_dims * num_sample if use_points_embed else embed_dims
        if use_camera_embed:
            ...
            if use_points_embed:
                self.weights_fc = nn.Sequential(
                    ...
                    nn.ReLU(),
                    nn.Linear(input_dims // 2, num_groups * num_levels * self.num_pts),
                    nn.ReLU(),
                    nn.Linear(num_groups * num_levels * self.num_pts, num_groups * num_levels * self.num_pts),
                )
            else:
                self.weights_fc = Linear(input_dims, num_groups * num_levels * self.num_pts)
        else:
            self.camera_encoder = None
            self.weights_fc = Linear(input_dims, num_groups * num_cams * num_levels * self.num_pts)
    ...
    def forward(
        ...
        **kwargs: dict,
        ...
        key_points = self.kps_generator(anchor, anchor_embed, instance_feature)
    ...
    def _get_weights(self, instance_feature, anchor_embed, metas=None):
        bs, num_anchor = instance_feature.shape[:2]
        if self.use_points_embed:
            feature = instance_feature.repeat(1, self.num_sample, 1) + anchor_embed
            if self.camera_encoder is not None:
                camera_embed = self.camera_encoder(metas["projection_mat"][:, :, :3].reshape(bs, self.num_cams, -1))
                feature = feature[:, :, None] + camera_embed[:, None]
                feature = feature.view(bs, num_anchor, self.num_sample, self.num_cams, -1)
                feature = feature.permute(0, 1, 3, 2, 4).reshape(bs, num_anchor, self.num_cams, -1)
            else:
                feature = feature.view(bs, num_anchor, self.num_sample, self.num_cams, -1)
                feature = feature.permute(0, 1, 2, 3).reshape(bs, num_anchor, -1)
        else:
    ...
    def feature_sampling(
        ...
        features = features.reshape(bs, num_cams, num_levels, -1, num_anchor, num_pts).permute(0, 4, 1, 2, 5, 3)
...
@PLUGIN_LAYERS.register_module()
class CustomOperation(BaseModule):
    def __init__(self):
        super().__init__()
        self.identity = nn.Identity()

    def forward(self, feature):
        return self.identity(feature)