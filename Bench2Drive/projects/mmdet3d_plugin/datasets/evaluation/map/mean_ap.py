...
def format_res_gt_by_classes(result_path,
    ...
    samples = [(gen, ann) for gen, ann in zip(gen_results.values(), annotations)]

    for i, clsname in enumerate(cls_names):
        print("processing {} ...".format(clsname))
        fn = partial(get_cls_results,
                     num_sample=num_fixed_sample_pts,
                     num_pred_pts_per_instance=num_pred_pts_per_instance,
                     eval_use_same_gt_sample_num_flag=eval_use_same_gt_sample_num_flag,
                     class_id=i,
                     fix_interval=fix_interval)
        gengts = pool.starmap(fn, samples)

        # gengts = []
        # sample_bar = mmcv.ProgressBar(len(annotations))
        # for sample, anno in zip(gen_results.values(), annotations):
        #     gengt = get_cls_results(
        #         gen_results=sample,
        #         annotations=anno,
        #         num_sample=num_fixed_sample_pts,
        #         num_pred_pts_per_instance=num_pred_pts_per_instance,
...
def print_map_summary(mean_ap,
    ...
    header = ['class', 'gts', 'dets', 'precis', 'recall', 'ap']
    for i in range(num_scales):
        if scale_ranges is not None:
