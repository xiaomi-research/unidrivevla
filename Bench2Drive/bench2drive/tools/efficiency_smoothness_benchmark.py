...
def read_from_json(filepath, metric_dir=None):
    ...
    for record in data["_checkpoint"]["records"]:
        matches = glob.glob(os.path.join(metric_dir, '*' + record["save_name"], 'metric_info.json'))
        filepath = matches[0] if matches else os.path.join(metric_dir, record["save_name"], 'metric_info.json')
        temp_dict = {}
...
if __name__=='__main__':
    ...
    argparser.add_argument('-f', '--file', required=True, help='path to merged.json')
    argparser.add_argument('-m', '--metric_dir', required=True, help='path to evaluation output directory')
